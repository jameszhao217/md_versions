...
!ER- I made these to split Al into top and bottom regions
	...
	integer :: i_james, ii_james, james_atoms
	integer :: james_type, james_num, MZ_Anneal_Step, MZ_chk_cyc
	double precision :: james_xpos, james_ypos, james_zpos
	...
	logical :: NeedList,EAMdebug,switchok,jamesrun
...
!	if(rank.eq.0) write(*,*)"call ReadMDinp()"
	...
	if(MDSteps.eq.0)then
		...
		nFCGcycle = 0.5
		writefiles = .true.
		MZ_chk_cyc = MZ_chk_cyc + 1
		MZ_Anneal_Step = -2
		if(rank.eq.0) then
			print *,'MZ_chk_cyc_ini',MZ_chk_cyc
			print *,'MZ_Anneal_Step_ini',MZ_Anneal_Step
		endif
		if(rank.eq.0)then
		...
		if(time.ne.loadLast)then
			...
			if(deltK*loadChange.lt.(0.d0))then
				...
				if(rank.eq.0) then
					print *,'MZ_MDSteps_ini',MDSteps
				endif


				MZ_chk_cyc = MZ_chk_cyc + 1
				if( (MZ_chk_cyc.eq.2) .or. (MZ_chk_cyc.eq.3))then
					MZ_Anneal_Step = MDSteps
				elseif( (MOD(MZ_chk_cyc,8).eq.0) )then ! Peak
					MZ_Anneal_Step = MDSteps
				elseif( (MOD(MZ_chk_cyc,8).eq.1) )then ! Valley
					MZ_Anneal_Step = MDSteps
				endif

				if(rank.eq.0) then
					print *,'MZ_chk_cyc',MZ_chk_cyc
					print *,'MZ_Anneal_Step',MZ_Anneal_Step
				endif
...
!	Set the MDStep at which we want annealing to occur
	if(MDSteps.eq.50)then

! 		Write the current atom positions to a file
		if(rank.eq.0) then
			open(unit=9222, file='James_Annealing',status='new')
			james_atoms = 0
			do i_james = 1, numnp
				if(IsRelaxed(i_james).eq.1) then
					james_atoms = james_atoms+1
					! write(9222,*) 'bulk atoms,x,y,z'
					write(9222,'(I6,1X,I6,1X,E27.17,1X,
     &  		E27.17,1X,E27.17)')
     & 			i_james,IsRelaxed(i_james),
     & 			atomCoord(1,i_james)+atomDispl(1,i_james),
     & 			atomCoord(2,i_james)+atomDispl(2,i_james),
     & 			atomCoord(3,i_james)+atomDispl(3,i_james)
				elseif(IsRelaxed(i_james).eq.2) then
					james_atoms = james_atoms+1
					! write(9222,*) 'interface atoms,x,y,z'
					write(9222,'((I6,1X,I6,1X,E27.17,1X,
     & 			E27.17,1X,E27.17))') 
     & 			i_james,IsRelaxed(i_james),
     & 			atomCoord(1,i_james)+atomDispl(1,i_james),
     & 			atomCoord(2,i_james)+atomDispl(2,i_james),
     & 			atomCoord(3,i_james)+atomDispl(3,i_james)
				elseif(IsRelaxed(i_james).eq.-1) then
					james_atoms = james_atoms+1
					! write(9222,*) 'pad atoms,x,y,z'
					write(9222,'((I6,1X,I6,1X,E27.17,1X,
     & 			E27.17,1X,E27.17))')
     & 			i_james,IsRelaxed(i_james)+4,
     & 			atomCoord(1,i_james)+atomDispl(1,i_james),
     & 			atomCoord(2,i_james)+atomDispl(2,i_james),
     & 			atomCoord(3,i_james)+atomDispl(3,i_james)
				endif
...
!		Print number of atoms interested to file 'output'	
		if(rank.eq.0) then
			print *,'NumJAMES',james_atoms
		endif

! 		Write the control file, will change 
! 		this number after finishing annealing
		if(rank.eq.0) then
			open(unit=438, file='James_ctrl', status='new')
			write(438,*) 0
			close(438)
		endif

!		Send CADD to an infinite loop to complete annealing
		jamesrun = .true.
		do while (jamesrun)
			call sleep(10)
			if(rank.eq.0) then
				open(unit=438, file='James_ctrl', status='old')
				read(438,*) ctrl_val
				if(ctrl_val.ne.0) then
					jamesrun = .false.
				endif
				close(438)
			endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	! Check Control file, if yes (1) start Annealing every fourth Cycle
	elseif(MDSteps.eq.MZ_Anneal_Step)then
	...
	if(rank.eq.0) then
		...
		if(ctrl_val.eq.1)then

			! Group atoms
			str001=''
			write(str001,*) 'group acti id < 20370'
			CALL lammps_command(lmp,str001)

			! Initialize and Ramp up the Velocities of atoms
			str001=''
			write(str001,*) 'velocity acti create 10.0 4928459
     &                   rot yes dist gaussian'
			CALL lammps_command(lmp,str001)

			str001=''
			write(str001,*) 'fix 1 all nve'
			CALL lammps_command(lmp,str001)
			str001=''
			write(str001,*) 'fix 3 all langevin 10.0 500.0 1.0 48279'
			CALL lammps_command(lmp,str001)
			str001=''
			write(str001,*) 'fix 4 pad setforce 0.0 0.0 0.0'
			CALL lammps_command(lmp,str001)
			str001=''
			write(str001,*) 'run 100000'
			CALL lammps_command(lmp,str001)
			...
			! Assign Temperature Back to Zero
			str001=''
			write(str001,*) 'fix 1 all nve'
			CALL lammps_command(lmp,str001)
			str001=''
			write(str001,*) 'fix 3 all langevin 500.0 0.0 1.0 48279'
			CALL lammps_command(lmp,str001)
			str001=''
			write(str001,*) 'fix 4 pad setforce 0.0 0.0 0.0'
			CALL lammps_command(lmp,str001)
